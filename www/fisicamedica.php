<?php

# Inicializa variáveis de menu
require_once ("includes/resetvars.php");
# Pega nome da página para ativar o menu CSS
$cursos = 'class="active"';

# Inclui cabeçalho
include("includes/header.php");
?>
		<!-- start content -->
		<div id="content">
			<div class="post">
					<h1 class="title">Bacharelado em Física Médica</h1>
					
					<h2 class="title">Descrição do Curso</h2>
					
					<div class="entry">
						
						<p>Bacharelado em Física Médica é um curso de graduação da Universidade de São Paulo – USP, 
						sediado na Faculdade de Filosofia, Ciências e Letras de Ribeirão Preto – FFCLRP, Campus Ribeirão Preto. 
						O Curso de Física Médica e demais cursos da USP são públicos e gratuitos.</p>
						
						<p>A graduação em Física Médica foi estabelecida para atender aos avanços tecnológicos que ocorrem na saúde. </p>
						
						<p>O currículo de Física Médica alia a formação do bacharelado em física como disciplinas de física básica, cálculo, 
						com disciplinas de formação específica como radioterapia, radiodiagnóstico, imagens médicas, medicina nuclear e ultrassom. </p>
						
						<p>Ao final do curso o estudante realizará um estágio em laboratórios de física, empresas, clínicas médicas ou hospitais, 
						aliando os conhecimentos específicos e profissionalizantes adquiridos durante o curso. </p>
						
						<p>Ao escolher a carreira de Física Médica (40 vagas/ano), o estudante do Ensino Médio deverá fazer o vestibular da FUVEST. 
						No site <a href="http://www.fuvest.br">www.fuvest.br</a> se encontram datas e informes sobre as inscrições e provas do vestibular. O estudante pode solicitar 
						isenção total da taxa de inscrição do vestibular, mas deve ficar atento aos prazos divulgados no site da FUVEST. </p>
						<div>
							<img src="img/alunos_sala.jpg" alt="Imagem Alunos Sala de Aula"/>
							<p class="legenda">Estudantes de Física Médica do 1º semestre assistindo a disciplina “Física – Modelos e Aplicações”.</p>
						</div>
					</div>
					
					
					<h2 class="title">Professores</h2>
					
					<div class="entry">
					
						<p>Os professores do curso atuam em diversas áreas de pesquisa ligadas à Física aplicada na Medicina, 
						na Biologia ou outras áreas da saúde, propiciando aos estudantes vivenciarem problemas reais inerentes à área de formação.</p>
						
						<p>Neste ambiente os alunos se engajam durante o curso de graduação em iniciações científicas, estágios remunerados, 
						profissionalizantes, bolsas de monitoria e outras atividades que preparam o aluno para o mercado de trabalho.</p>
						
					</div>
					
					<h2 class="title">Áreas de Atuação</h2>
					
					<div class="entry">
						<p>O Bacharel em Física Médica pode atuar em diferentes áreas relacionadas à Física Médica, Biomédica e demais áreas 
						tradicionais da Física; em programas de Pós-Graduação de diferentes Universidades Brasileiras ou no Exterior, 
						Hospitais e Clínicas médicas, empresas multinacionais que desenvolvem ou comercializam equipamentos médicos hospitalares.</p>
					</div>
							
					<div>
						<img src="img/alunos_lab.jpg" alt="Imagem Alunos no Laboratório"/>
						<p class="legenda">Estudantes do 6º semestre de Física Médica no Laboratório de Eletrônica e Instrumentação Biomédica.</p>
					</div>
					
					<h2 class="title">Infraestrutura do curso</h2>
					
					<div class="entry">
						<p>O <a href="http://df.ffclrp.usp.br/">Departamento de Física</a> sedia o Curso de Física Médica na FFCLRP e está equipado com 
						laboratórios de física básica, física moderna, eletrônica, instrumentação biomédica e dosimetria, além de algumas 
						disciplinas utilizarem equipamentos de Laboratórios de Pesquisa e do Hospital das Clínicas da Faculdade de Medicina de Ribeirão Preto. </p>
					
						<p>Os estudantes do Curso de Física Médica estudam em salas de aulas e laboratórios de ensino próximos aos laboratórios de pesquisa do 
						Departamento de Física. Isto possibilita uma grande interação com estudantes de pós-graduação, pesquisadores, participação em reuniões 
						de pesquisa e podem assistir palestras de Professores Brasileiros e Visitantes Estrangeiros expondo as atualidades da Física Médica no Brasil e no mundo.</p>
						
						<p>Os alunos contam com sala de estudos, bibliotecas, Centro Estudantil de Física Médica, o <a href="http://sites.ffclrp.usp.br/cefim">CEFIM</a>, 
						além de toda a infraestrutura disponível no Campus de Ribeirão Preto (www.ribeirao.usp.br) e demais Campi da <a href="http://www.usp.br">Universidade de São Paulo</a>. 
						Para melhor conhecer a infraestrutura que está disponível aos alunos da USP, visitem as páginas na internet.</p>
						
						<p>Após ingressar no curso os alunos podem participar de intercâmbios internacionais, em Universidades estrangeiras ou em eventos internacionais, 
						ter bolsa de iniciação científica, estágios numerados e ter todo o suporte do Programa de Permanência Estudantil da USP com auxílio Moradia, Alimentação, 
						Transporte e diversas bolsas remuneradas para garantir a permanência de estudantes na Universidade de São Paulo. </p>
						
						<p>Para saber mais sobre o ingresso na USP acesse: 
						
						<br /><a href="http://www.prpg.usp.br" target="_blank">Programa de Pós-Graduação</a>
						
						<br /><a href="http://www.facebook.com/vocetambempodeUSP" target="_blank">Página do Facebook</a>
						
						</p>
						
											
					</div>
				</div>
			</div>
		<!-- end content -->

